(ns switchboard.actor
  "TelephoneSwitchboardOperatorsActor — the ISCO-08 4223 community switchboard actor as a
  `langgraph.graph/state-graph` (ADR-2607011000 / CLAUDE.md Actors
  section). One graph run = one switchboard operation request
...
(defn build-graph
  "Build a compiled TelephoneSwitchboardOperatorsActor graph. `store` implements
  `switchboard.store/Store`. `advisor` implements
  `switchboard.advisor/Advisor` (defaults to `mock-advisor`).
